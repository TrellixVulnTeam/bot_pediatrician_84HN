...
Берегите себя. В аптечке держите маленький пузырек масла чёрного тмина. На стадии заживления прекрасно справляется,без шрамов.''')
        ...
        bot.send_message(message.chat.id, '''
Отипакс не доказал своей эффективности, так же его нельзя использовть не убедившись в целостности барабанной перепонки.
Первая помощь - это обезболевающее ибупрофен/парацетамол в возрастной дозировке, системный антибиотик по назначению врача.
При прорезывании зубов также бывает ушная боль, даже если не красно там нигде.
